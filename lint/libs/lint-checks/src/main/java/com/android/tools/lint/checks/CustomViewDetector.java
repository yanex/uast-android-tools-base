...
public class CustomViewDetector extends Detector implements Detector.UastScanner {
    ...
    public void visitFunctionCallExpression(@NonNull JavaContext context,
        ...
        if (skipParentheses(call.getParent()) instanceof PsiExpressionStatement) {
            ...
            UClass cls = UastUtils.getParentOfType(call, UClass.class, false);
            if (cls == null) {
                return;
            }

            String className = cls.getName();
            if (cls.isSubclassOf(CLASS_VIEW, false)) {
                if (!styleableName.equals(className)) {
                    ...
                    context.report(ISSUE, call, context.getLocation(expression), message);
                }
            } else if (cls.isSubclassOf(CLASS_VIEWGROUP + DOT_LAYOUT_PARAMS, false)) {
